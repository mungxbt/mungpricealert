...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Crypto Alert Bot by mungxbt | @simpulcrypto\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "💰 HARGA\n"
        "/price BTC          → harga USD\n"
        "/price BTC IDR      → harga Rupiah\n\n"
        "🔔 PRICE ALERT\n"
        "/alert BTC 90000    → set alert\n"
        "/listalerts         → lihat alert aktif\n"
        "/removealert BTC 90000  → hapus spesifik\n"
        "/removealert BTC    → hapus semua BTC\n\n"
        "📊 FUNDING & OI\n"
        "/funding BTC        → cek funding rate\n"
        "/addfunding BTC     → monitor spike funding\n"
        "/removefunding BTC  → hapus monitor\n"
        "/listfunding        → lihat monitor aktif\n"
        "/oi BTC             → cek open interest\n"
        "/addoi BTC          → monitor spike OI\n"
        "/removeoi BTC       → hapus monitor\n"
        "/listoi             → lihat monitor aktif\n\n"
        "📈 MARKET\n"
        "/lsr BTC            → long/short ratio\n"
        "/topgainers         → top 5 naik 24h\n"
        "/toplosers          → top 5 turun 24h\n"
        "/feargreed          → fear & greed index\n"
        "/dominance          → BTC & ETH dominance\n"
        "/heatmap            → coin paling ramai\n\n"
        "🦎 DEX & MEMECOIN\n"
        "/dex PEPE           → cari token by nama\n"
        "/dex solana <CA>    → cari by contract\n"
        "/dexalert <CA> <mcap>   → alert market cap\n"
        "/listdexalerts      → lihat dex alert\n"
        "/removedexalert <CA>    → hapus alert\n"
        "/dextrending        → token paling di-boost\n"
        "/dexnew             → token baru listing\n\n"
        "📣 CALL TRACKER\n"
        "/buy BTC 75000 TP 80000 SL 73000   → long call\n"
        "/sell BTC 75000 TP 70000 SL 78000  → short call\n"
        "/mycalls            → call aktif lo\n"
        "/allcalls           → semua call di grup\n"
        "/removecall BTC     → hapus call\n"
        "/stats              → statistik global\n"
        "/stats me           → statistik lo\n"
        "/stats @budi        → statistik orang lain\n"
        "/stats 2025-03      → statistik bulan tertentu\n"
    )
